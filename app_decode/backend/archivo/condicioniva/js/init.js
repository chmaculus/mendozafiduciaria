$(document).ready(function(){
    ...
    $('#send').on('click', function(event) {
       ...
       if ( !$("#customForm").validationEngine('validate') )
        ...
        $.ajax({
            url : _condicioniva.URL + "/x_sendobj",
            data : {
                obj:obj
            },
            dataType : "json",
                        ...
                        $.unblockUI({
                            onUnblock: function(){ 
                                jAlert('Operacion Exitosa.', $.ucwords(_etiqueta_modulo),function(){
                                    ...
                                    RefreshTable('#datatable', _condicioniva.URL +"/x_get_datatable");
                                });
                            } 
                        }); 
                    }, 500);
    ...
    $('.btnBorrar').on('click', function(e) {
        ...
        jConfirm('Esta seguro de borrar este item??.', $.ucwords(_etiqueta_modulo),function(r){
          if(r==true){
              ...
              //eliminar
              $.ajax({
                  url : _condicioniva.URL + "/x_delobj",
                  data : {
                      id : id
                  },
                  type : "post",
...
function cargarResults(){
    ...
    var settings = {
        ...
        "sAjaxSource": _condicioniva.URL + "/x_get_datatable",
...
function editar(id){
    ...
    $.ajax({
        url : _condicioniva.URL + "/x_getobj",
        async: false,
        data : {
            id : id
        },
