...
class condicioniibb extends main_controller{
    function condicioniibb(){
        $this->mod = $this->model("condicioniibb_model");
    }
    ...
    function init(){
        ...
        $id_permiso = 4; // permiso de acceso a este modulo (fid_permisos)
        ...
        $datax['etiqueta_modulo'] = "Condición IIBB";
    ...
    function _obtener_main($arr_permiso_mod){
        ...
        $data['modulo'] =  "";
        $data['etiqueta_mod'] =  "Condición IIBB";
        /* permiso mostrar */
        if($_SESSION["USER_ROL"]==1 || $arr_permiso_mod['MOSTRAR'] == 1)
            return $this->view("condicioniibb", $data);
        else
