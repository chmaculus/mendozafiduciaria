...
class carpetash1 extends main_controller{
    function carpetash1(){
        $this->mod = $this->model("carpetash1_model");
    }
    ...
    function init($val_init='',$val_id=''){
        ...
        //log_this('log/rrrrrrr.log',$val_init . "--1111");
    ...
    function x_sendobj(){
        ...
        $arr_desem = isset($obj['arr_desem'])?$obj['arr_desem']:array();
        $rand = isset($obj['rand'])?$obj['rand']:array();
        unset($obj['tipo_seleccion'],$obj['POSTULANTES'],$obj['CARTERADE'],$obj['adjuntos'],$obj['rand'],$obj['arr_desem']);
        $rtn = $this->mod->sendobj( $obj, $postulantes, $obj["ID_ETAPA_ACTUAL"], $carterade, $adjuntos, $rand, $arr_desem );
    ...
    function x_getform_addentidad(){
        ...
        if ($tmp){
            ...
            //desembolsos
            $lst_desemb = $this->mod->get_desemb_ope($tmp[0]['ID']);
            $data['lst_desemb'] = $lst_desemb;
