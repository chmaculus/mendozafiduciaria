<?php
class carpetash1_model extends main_model{
...
//        log_this('eeeeee.log', $this->_db->last_query());
    ...
    function sendobj($obj,$postulantes,$id_etapa_actual,$carterade,$adjuntos,$rand,$arr_desem){
        $fecha_actual = date("Y-m-d H:i:s");
        ...
        $obj['FECHA'] = deFecha_a_base($obj['FECHA']);
        ...
        if ($iid==0)://agregar
            ...
            log_this('log/qqqqqq.log', $this->_db->last_query() );
            log_this('log/zzzzzz.log', print_r($obj,1));
            ...
            $acc = "add";
            $id_new = $resp;
            //$this->procesar_etapas($id_new,$modo='agregar',$obj['ID_PROCESO']);
            ...
            $this->_db->update( 'fid_garantias', array("ID_OPERACION"=>$id_new), "OBSERVACION='".$rand."'" );
            
            $estado_credito = 0;
            if($obj['ID_ESTADO']==2):
                $estado_credito = ESTADO_CREDITO_VIGENTE;
            elseif($obj['ID_ESTADO']==29):
                $estado_credito = ESTADO_CREDITO_CANCELADO;
            endif;
            ...
            $objcred = array(
                            "ID_OPERACION"=>$resp,
                            "ID_FIDEICOMISO"=>$obj['ID_FIDEICOMISO'],
                            "ID_OPERATORIA"=>$obj['ID_OPERATORIA'],
                            "CAPITAL_VTO"=>date("Y-m-d H:i:s"),
                            "CREDITO_ESTADO"=>ESTADO_CREDITO_VIGENTE
                        );
            
            $post = "";
            if($postulantes):
                $objcred["POSTULANTES"] = $postulantes[0];
            endif;
            $cred = $this->_db->insert('fid_creditos', $objcred );
                        
        else://editar
            
            unset($obj["OBSERVACIONES"],$obj["FEC"],$obj["ESTADO"]);
            
            if (isset($new_arr_chk["chk_checklist"]) && $new_arr_chk["chk_checklist"]==1 && $_SESSION["USER_ROL"]==9){
            ...
            //$iid
            $this->_db->select('ID');
            $cred = $this->_db->get_tabla('fid_creditos', "ID_OPERACION='".$iid."'");
            if ($cred){
                $cred = $cred[0]["ID"];
            }
        ...
        if ($arr_desem):
            ...
            foreach ($arr_desem as $valor):
                
                if(intval($valor["anio"])>0):
                    $mk = mktime(0, 0, 0, $m, $d, $valor["anio"]);
                else:
    ...
    function get_desemb_ope($idope){
        //SELECT * FROM `fid_creditos_desembolsos` WHERE id_credito in ( select id from fid_creditos where id_operacion='50' order by id desc )
        $rtn = $this->_db->get_tabla("fid_creditos_desembolsos", "id_credito in ( select id from fid_creditos where id_operacion='".$idope."' order by id desc )");
        return $rtn;
    }
