...
class operatorias extends main_controller {
    ...
    function x_getform_addentidad() {
        ...
        if ($tmp) {
            ...
            $data['lst_cambiotasas'] = $this->mod->get_cambiotasas_x_op($obj);
    ...
    function x_impactar_tasas() {
        ...
        if ($impactar_tasas) {
            ...
            if ($id = $this->mod->guardar_cambio_tasa($obj, $fecha_imp_tasas)) {
                $result['result'] = TRUE;
                $result['id'] = $id;
                $result['creditos'] = $this->mod->get_creditos_operatoria($obj['id'], $fecha_imp_tasas);

                $data = array();
                $data['lst_cambiotasas'] = $this->mod->get_cambiotasas_x_op($obj['id']);
                $data['imputacion_tasas'] = TRUE;
    ...
    function x_sinc_tasas() {
        $result = array ('result' => FALSE);
        if ($cambiotasa = $this->mod->get_cambiotasas($_POST['id'])) {
            if ($creditos = $this->mod->sinc_tasas($cambiotasa['ID_OPERATORIA'], $cambiotasa['FECHA'])) {
                $result = array('result' => TRUE, 'creditos' => $creditos);
            }
        }
    ...
    function x_get_cambiotasa() {
        $result = array('result' => FALSE);
        if ($cambiotasa = $this->mod->get_cambiotasas($_POST['id'])) {
            $data = array();
            $data['lst_cambiotasas'] = $this->mod->get_cambiotasas_x_op($cambiotasa['ID_OPERATORIA']);
            $data['imputacion_tasas'] = TRUE;
