...
//        resetlog();
    ...
    function borrar_credito() {
        // echo "llega borra".$this->_id_credito;
    ...
    function _make_segmento_cuota($cuota_id, $fecha = NO_FECHA) {
        ...
        if ($cuota) {
            ...
            //  $fecha_enviada = false;//$cuota['FECHA_ENVIADA'] > 0 ? $cuota['FECHA_ENVIADA'] : false;            
            ...
            if ($subcuotas) {
                ...
                if ($cuota['CUOTAS_RESTANTES']==17){
                   // echo "CANTIDAD SUBCUOTAS:".count($subcuotas)." -";
                }                    
                    ...
                    if ($bprimer_segmento) {
                        ...
                        if ($cuota['CUOTAS_RESTANTES'] == 16) {
                            //print_array( $subcuotas[$i - 1]['TMP']['VARIACION']);
                        //    echo "INTERES COMP:" . $INTERES_COMPENSATORIO_VARIACION . "-";
                        }
                    ...
                    if ($cuota['CUOTAS_RESTANTES'] == 16) {
                        //print_array( $tmp);
                        //echo "INTERES COMP:" . $INTERES_COMPENSATORIO_VARIACION . "-" . date("d/m/Y", $fecha_vencimiento_rango);
                    }
                    ...
                    if (!$not_enter && ($tmp['_ACTIVA'] == -2 || ($fecha_vencimiento == $cuota['FECHA_VENCIMIENTO'] && $fecha_get > $fecha_vencimiento) )) {
                        ...
                        if (round($SALDO_CUOTA) == 0 || round($rango) == 0) {
                            ...
                            if ($this->_tipo_credito == TIPO_MICROCREDITO && false) {
                                ...
                                if ($cuota['CUOTAS_RESTANTES']==16) {
                                    //print_array($cuota);
                                    /*echo " FECHA:".date("d/m/Y",$cuota['FECHA_VENCIMIENTO']);
                                    echo "- POR MORATORIO:".$cuota['POR_INT_PUNITORIO'];
                                    echo "- RANGO:".$rango;
                                    
                                    echo "- PLAZO:".$this->_interese_punitorio_plazo;
                                    echo "- SALDOx--:".$SALDO_CUOTA."--<br/>";*/
                                    //echo "IC:".$INTERES_COMPENSATORIO2 ."+ IVIC:". $IVA_INTERES_COMPENSATORIO2 ."-PAGOS:". $total."||";
                                    //echo "(".$SALDO_CUOTA ."*(1 + (".$cuota['POR_INT_MORATORIO']." / 100) * ".$rango." / ".$this->_interese_moratorio_plazo." ) - ".$SALDO_CUOTA." = ".$INT_MORATORIO.")<br/>";
                                }
            ...
            foreach ($CHILDRENS_TMP as $CH_TMP) {
                ...
                if ($CH_TMP['FECHA_VENCIMIENTO'] >= $VERSION_FECHA_INICIO)
                    break;
    ...
    function _join_segmento_cuota($cuota_id, $fecha_actual = false) {
        ...
        if ($bsegmentos) {
            ...
            foreach ($segmentos as $segmento) {
                ...
                if ($cuota['CUOTAS_RESTANTES'] == 1) {
                    //echo "llega:". $segmento['INT_COMPENSATORIO_SUBSIDIO'];
                }
            ...
            //echo $cantidad_cuotas_anteriores ."-". $CANTIDAD_CUOTAS_GRACIA."||";
    ...
    function _calcular_interes($monto, $dias, $interes = 10, $periodicidad = 60, $log = false) {
        ...
        if ($log) {
            //$cuenta = $monto . " * pow( ( 1 + ((" . $interes . " * " . $periodicidad . " / " . $this->_interese_compensatorio_plazo . "))), (" . $dias . " / " . $periodicidad . ") ) - " . $monto;
            //echo "CUENTA:".$cuenta.":". (7500 * pow( ( 1 + ((0.09425 * 30 / 360))), (30 / 30) ) - 7500)."<br/>";
            //echo $monto . " - " . $dias . " - " . $interes . " - " . $periodicidad . " - " . $this->_interese_compensatorio_plazo . " = " . $rtn . "<br/>";
        }
    ...
    function enviar_cuota() {
        ...
        $cuota_enviar = array();
        foreach ($this->_cuotas as $cuota) {
            //echo date("d/m/Y", $fecha) . " - " . date("d/m/Y", $cuota['FECHA_VENCIMIENTO']) . "<br/>";
    ...
    //mueve la fecha de planchado de la cuota al ultimo evento registrado en la misma
    /* function mover_envio_ultimo_evento($fecha){

      $id_cuota = 0;
      //buscamos la cuota por fecha
      foreach($this->_cuotas as $cuota){
      if ($fecha < $cuota['FECHA_VENCIMIENTO'] && $fecha > $cuota['FECHA_INICIO']){
      $id_cuota = $cuota['ID'];
      }
      }

      if ($id_cuota==0) return 0;



      $cuota = $this->_cuotas[$id_cuota];

      if ($cuota['FECHA_ENVIADA'] == 0) return 0;

      $fecha_envio = $cuota['FECHA_ENVIADA'];

      //recorremos los segmentos de las cuotas en busqueda de la ultima y cambiar ahi la fecha de envio
      foreach($this->_variaciones as $child){
      if (!($child['FECHA'] >= $cuota['FECHA_INICIO'] && $child['FECHA'] <= $cuota['FECHA_VENCIMIENTO']) )
      continue;


      //se comprueba que la fecha del evento sea mayor a la feha de envio existente
      if ($fecha_envio < $child['FECHA']){
      $fecha_envio = $child['FECHA'];
      }

      //si la fecha del evento es mayor a la fecha de vencimiento de la cuota no hay cambio
      if ($child['FECHA'] >= $cuota['FECHA_VENCIMIENTO']) break;

      }

      //se comprueba que se haya modificado la fecha de la cuota
      if ($fecha_envio > $cuota['FECHA_ENVIADA']){
      if ($this->_bsave){
      $this->_db->update("fid_creditos_cuotas",array("FECHA_ENVIADA"=>$fecha_envio), "ID = ".$id_cuota );
      }
      $this->_cuotas[$id_cuota]['FECHA_ENVIADA'] = $fecha_envio;
      return $fecha_envio;
      }
      else{
      $this->_cuotas[$id_cuota]['FECHA_ENVIADA'] = $fecha_envio;
...
ORDER BY T1.lvl DESC');
    ...
    /*
      ...
      //cambiamos el estado del pago realizado anterioremente
      $this->_db->update("fid_creditos_eventos", array("ESTADO" => -1), "ID = " . $variacion['ID']);
