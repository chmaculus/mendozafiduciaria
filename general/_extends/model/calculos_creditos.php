    ...
    function payDeuda($fecha, $monto = 0){
     ...
     //       if ($i++==7) die();
            ...
            foreach($pagos as $key=>$value){
                ...
                echo "pago:".$montoPagoItem." - ".$itemsPago[$key]."<br/>";
                ...
                if ($cuota->getCapital() && $key===PAGO_CAPITAL){
                    ...
                    if ($value===$montoPagoItem){
                        ...
                        if ($fecha < $cuota->getRango()->getEnd()){
                            echo "VENCIMIENTO NUEVO:";
                            $calculosCuota->changeVencimientoCuota($fecha);
                            echo date("d/m/Y",$cuota->getRango()->getEnd());
                        }
                    }
                }
    ...
    function getSaldoCapital($fecha = false, $log = false) {
        ...
        foreach ($cuotasCollectionFecha as $cuota) {
           ...
           // $AMORTIZACION_TEORICA_ACUMULADA = $cuota->getCapital();
            ...
            if ($log){
               // echo "::".date("d/m/Y",$cuota->getEventoVencimiento()->getFecha())."_".$cuota->getCapital()."-".$cuota->getCuotasRestantes()."<br/>" ;
            }
            if ($cuota->getCapital() > 0) {
                ...
                if ($log){
                   ...
                   // echo $divisor."-".date("d/m/Y",$cuota->getEventoVencimiento()->getFecha())."-".$AMORTIZACION_TEORICA_ACUMULADA."-".$AMORTIZACION_CUOTA;
                }
    ...
    function EventoFactory($cuotasCollection, $evento) {
        ...
        switch ($evento['TIPO']) {
            ...
            case EVENTO_DESEMBOLSO:
                ...
                $this->_evento = new EventoDesembolso($evento['FECHA'], $evento['ASOC']['MONTO']);
                $this->_evento->setID($evento['ID']);
                break;
            ...
            case EVENTO_TASA:
                ...
                $this->_evento = new EventoTasaModificacion($evento['FECHA'], $compensatorio, $punitorio, $moratorio, $bonificacion);
                $this->_evento->setID($evento['ID']);
                break;
            ...
            case EVENTO_RECUPERO:
                ...
                $this->_evento = new EventoRecupero($evento['FECHA'], 0);
