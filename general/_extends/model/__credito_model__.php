...
define("TIPO_MICROCREDITO",1000);
define("TIPO_CREDITO_NORMAL",2000);
    ...
    function verificiar_eventos_posteriores(){
        ...
        foreach($this->_variaciones as $variacion){
            if ($variacion['FECHA'] > $this->_fecha_actual && $variacion['TIPO']){
                if (!($variacion['TIPO']==1 && $variacion['ESTADO']==5) && $variacion['TIPO'] != 0 ){
                    return true;
                }
            }
        }
...
*/                
               ...
               /*     if ($cuota['FECHA_ENVIADA'] > 0) {
                        ...
                        if ( ($fecha >= $cuota['FECHA_ENVIADA'] || $this->_tipo_credito==TIPO_MICROCREDITO) ) {
       ...
       /*         if ($cuota['FECHA_ENVIADA'] > 0) {
            ...
            if ($cuota['FECHA_VENCIMIENTO'] < $fecha) {
                ...
                if ($cuota['ESTADO']==PLAZO_SUBSIDIO_VENCIDO && $cuota['INT_COMPENSATORIO_SUBSIDIO'] > 0){
                        ...
                        $arr_deuda['fecha_reimputacion'] = $cuota['FECHA_INICIO'];
                }
    ...
    function join_segmento_cuota($cuota_id, $fecha = false) {
        ...
        if ($bsegmentos){
            ...
            //si la fecha de calculo es anterior a la fecha de envio de cuota
            if ($fecha < $cuota['FECHA_ENVIADA']){
                $bsegmentos = true;
            }            
        ...
        //-existen segmentos y no es microcredito
        ...
        /*if ( ($segmentos && $this->_tipo_credito != TIPO_MICROCREDITO) || 
            ...
            if (true){
                $capital_arr = $this->_get_saldo_capital($cuota['FECHA_VENCIMIENTO'], true);
                $this->_cuotas[$cuota_id]['SALDO_CAPITAL'] = $capital_arr['SALDO'];
                //$this->_cuotas[$cuota_id]['INT_COMPENSATORIO_IVA'] = $cuota['INT_COMPENSATORIO_IVA'];     */
                //print_array($capital_arr);
            }            

            $SALDO_CAPITAL = $capital_arr['SALDO'];
            $cuota['CAPITAL_CUOTA'] = ($desembolsos + $inicial) / ( $cuota['CUOTAS_RESTANTES'] + $cantidad_cuotas_anteriores - $CANTIDAD_CUOTAS_GRACIA);
            $cuota['SALDO_CAPITAL'] = $SALDO_CAPITAL;
               ...
               /*     if ($cuota['ESTADO'] == 1)
                    ...
                    if (!$not_enter && ($tmp['_ACTIVA'] == -2 || ($fecha_vencimiento == $cuota['FECHA_VENCIMIENTO'] && $fecha_get > $fecha_vencimiento) )) {
                        ...
                        if ($fecha_get > $FIN_PLAZO_PAGO_SUBSIDIO && ($IVA_INT_SUBSIDIO_ACUMULADO + $INT_SUBSIDIO_ACUMULADO > 0) ) {
                            logthis("PLAZO PAGO: " . $cuota['CUOTAS_RESTANTES']."_".microtime(),date("d/m/Y",$fecha_get)." - ".date("d/m/Y",$FIN_PLAZO_PAGO_SUBSIDIO) );
...
//echo "(((CR:".$cuotas[$c]['CUOTAS_RESTANTES'] ." - C:".$c." - CG: ".$CUOTAS_GRACIA.")))";
            if ($c >= $CUOTAS_GRACIA) {
                $audi .= "<br/>CUOTAS GRACIA: NO";
                $divisor = $cuotas[$c]['CUOTAS_RESTANTES'];
              //  echo "(((".$DESEMBOLSOS_ACUMULADOS ."-". $AMORTIZACION_TEORICA_ACUMULADA." / ".$divisor.")))";
...
ORDER BY T1.lvl DESC');
    ...
    function agregar_version($fecha = false, $tipo = 1, $descripcion = "") {
        ...
        foreach ($variaciones as $variacion) {
            if ($variacion['FECHA'] >= $fecha) {
                ...
                if ($variacion['TIPO']!=3 && $variacion['TIPO']!=0){
    ...
    function renew_datos($fecha = NO_FECHA) {
        ...
        if ($this->_bsave) {
            ...
            $primer_cuota = reset($this->_cuotas);
            if ($primer_cuota['FECHA_ENVIADA']==$primer_cuota['FECHA_INICIO']){
                $this->_tipo_credito = TIPO_MICROCREDITO;
            }
    ...
    function get_desembolsos(){
        ...
        foreach($this->_variaciones as $variacion){
            if ($variacion['TIPO']==1 && $variacion['ESTADO']!=5){
                ...
                if ($variacion['ASOC']['MONTO']==0) continue;
    ...
    function get_pagos(){
        ...
        $i = 1;        
        foreach($this->_variaciones as $variacion){
            if ($variacion['TIPO']==3 AND $variacion['ESTADO']==0 ){
