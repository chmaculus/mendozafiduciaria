...
class CalculosCuota {
    ...
    function makeFIDEstado($fecha = false, $tipoDevengamiento = 1) {
        ...
        echo "<br/>CAPITAL:".$this->_cuota->getCapital()."-";
    ...
    private function _recursiveSegmentoResult(EventosCollection $eventos, EventoInicial $eventoTasa, &$detalleItemResultArray) {
        ...
        if ($eventoActual->getTipo() == EVENTO_INFORME) {
            echo "<br/>EVENTO INFORME:".date("d/m/Y",$eventoActual->getFecha()) ."<br/>";
            return;
        }
        ...
        if ($eventos->next()->valid()) {
            ...
            if ($eventoSiguiente->getFecha() <= $this->_cuota->getRango()->getEnd()) {
                ...
                $log = false;
                if (date("d/m/Y",$this->_cuota->getRango()->getEnd())==='30/08/2011'){
                 //   $log = true;
                }                 
                $resultSaldo = $calculosCredito->getSaldoCapital($eventoActual->getFecha(),$log);
                if (date("d/m/Y",$this->_cuota->getRango()->getEnd())==='30/05/2011'){
                }                 
                
                $monto_actual = $resultSaldo['BASE_CALCULO'];
                $monto_interes = $this->calcular_interes(
                        $monto_actual, $rango, $eventoTasa->getCompensatorio(), $this->_credito->getPeriodicidad(), $this->_credito->getCompensatorioPlazo());
                ...
                if ($this->_credito->getEventoInforme()->getFecha() <= $eventoActual->getFecha()) {
                    echo "<br/>FIN:".date("d/m/Y",$this->_credito->getEventoInforme()->getFecha())." - ".date("d/m/Y",$eventoActual->getFecha())."<br/>";
                    return;
                }          
                
                $log = false;
                if (date("d/m/Y",$this->_cuota->getRango()->getEnd())==='30/03/2011'){
                    $log = true;
                }                
                
                //la base de calculo se calcula al final de la cuota a partir del vencimiento
                //de la cuota
                $resultSaldo = $calculosCredito->getSaldoCapital($this->_cuota->getRango()->getEnd()-1, $log);
                if (date("d/m/Y",$this->_cuota->getRango()->getEnd())==='30/03/2011'){
                ...
                $detalleItemResultPago = $this->getRecuperoItemResult($eventoSiguiente->getFecha());
                ...
                $log = false;
                if (date("d/m/Y",$this->_cuota->getRango()->getEnd())==='30/04/2011'){
          //          print_array($resultSaldo);
       //             echo "FECHA: ".date("d/m/Y",$eventoSiguiente->getFecha())." - RANGO: ".$rango." - AMORTIZACION: ".$resultSaldo['AMORTIZACION_CUOTA'] ."<br/>";
                }                
                ...
                if ($eventoTasa->getBonificacion() > 0){
                    ...
                    if ($difDias > $diasVencimiento){
                        ...
                        return;
                /*        $eventoTasa = $this->_cuota->getEventoTasaModificacion();
                        $bonificacion = 0;
                        $punitorio = $eventoTasa->getPunitorio();
                        $moratorio = $eventoTasa->getMoratorio();
                        $compensatorio = $eventoTasa->getCompensatorio();
                        $fechaEventoNuevo = $this->_cuota->getRango()->getStart();
                        $tasaEventoSinBonificacion = new EventoTasaModificacion(
                                $fechaEventoNuevo,
                                $compensatorio,
                                $punitorio,
                                $moratorio,
                        ...
                        $this->_eventos->addEvento($tasaEventoSinBonificacion);*/
                    }
                }                
...
/*
                                    echo " <br/>FECHA:".date("d/m/Y",$eventoSiguiente->getFecha());
                                    echo "- POR PUNITORIO:".$eventoTasa->getPunitorio();
                                    echo "- RANGO:".$rango;
                                    
                                    echo "- PLAZO:".$this->_credito->getMoratorioPlazo();
                                    echo "- PLAZO:".$this->_credito->getPunitorioPlazo();
                                    echo "- SALDO:".$saldoCuota."";
                                    echo "- TOTAL:".$monto_moratorio."|";                
                                    echo "- TOTAL:".$monto_punitorio."<br/>";             */   
            }
